...
class bart_classifier(nn.Module):
    ...
    def __init__(self, num_labels, model_select, gen, dropout, dropoutrest):
        ...
        # self.l1_linear = nn.Linear(self.bart.config.hidden_size, self.bart.config.hidden_size, bias=False)
        # self.tanh = nn.Tanh()
        # self.l2_linear = nn.Linear(self.bart.config.hidden_size, num_labels, bias=False)
        # self.softmax = nn.Softmax(dim=1)

        # self.fc = nn.Sequential(nn.Linear(self.bart.config.hidden_size, self.bart.config.hidden_size), nn.Dropout(0.2),
        #                         nn.Linear(self.bart.config.hidden_size, num_labels))
        # self.atn = ATN()
    ...
    def forward(self, **kwargs):
        ...
        # att_hidden = last_hidden[0][:batch_size, ::]
        # l1_ouput = self.tanh(self.l1_linear(att_hidden))
        # att_wei = self.softmax(self.l2_linear(l1_ouput)).transpose(1, 2)
        # att_out = torch.matmul(att_wei, att_hidden)
        # att = att_out.sum(dim=2)
        ...
        # fc_input = self.atn(linear1,linear2,linear2)
