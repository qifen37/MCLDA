        ...
        for epoch in range(0, int(config['total_epochs'])):
            ...
            for b_id, sample_batch in enumerate(trainloader):
                ...
                if step%split_step==0:
                    model.eval()
                    with torch.no_grad():
                        preds_train, loss_train_inval_mode = model_utils.model_preds(trainloader2, model, device, loss_function,cl_loss_cunction)#train2和train数据一样
                        preds_val, loss_val = model_utils.model_preds(valloader, model, device, loss_function,cl_loss_cunction)
                        preds_test, loss_test = model_utils.model_preds(testloader, model, device, loss_function,cl_loss_cunction)
                        print(100*"#")
                        ...
                        train_writer.add_scalar('loss', sum(train_loss)/len(train_loss), step)#将训练损失的平均值记录到TensorBoard中
